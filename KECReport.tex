...
\begin{itemize}
	\item One-Way Property: For given h ∈ {0, 1}^n, it is practically impossible to find a value m ∈ {0, 1}∗
	with H(m) = h. 
	\item 2nd-Preimage-Property: For given m ∈ {0, 1}^∗, it is practically impossible to find a value m ∈
	{0, 1}^∗\{m} with H(m) = H(m′)
	\item Collision Resistance: It is practically impossible to find two values m, m ∈ {0, 1}∗ with m ̸= m
	and H(m) = H(m′).
\end{itemize}
...
n ≥ 240 must be imposed on hash function H : {0, 1}∗ → {0, 1}n because of the birthday problem.
...
digest length of ≥ 256 bits. \cite{bsi2020}
...
\begin{itemize}
	...
	\item Verify the received public key: The receiver desalts the received key using the salt information (salt value & position) in the database 
