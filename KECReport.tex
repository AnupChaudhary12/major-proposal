...
Since we used RSA algorithm for securing the data.\cite{sa}
...
\begin{itemize}
	...
	\item It provides encryption, decryption to messages.
\end{itemize}
...
\subsection{Diffie-Helmann Algorithm}
With simple encryption, the messages are usually encrypted only between the users and the server, making use of some cryptographic keys, hence making data vulnerable at the server. We want these keys only to exist between the users and not the server. Diffie-Helmann makes this possible. Suppose we have two clients ALice and Bob.

\vspace{-18pt}
\begin{itemize}
	\item Alice and Bob agree to use two common prime numbers (g \& n) provided by the server
	\item Now, these are combined using some mathematical calculations with the Private keys of Alice and Bob = a + g = ag and b + g = bg.
	\item We exchange these Ephemeral/Public Keys ag and bg via server.
	\item Combine the exchanged keys with the Private keys of Alice and Bob respectively to form a Shared Secret Key = ag+b = agb and bg+a = bga at both ends.
	\item Now the attacker might be aware of g, n, ag \& bg as these are being shared publicly, but not a \& b since these are private keys only available to Alice and Bob.
	\item It is too difficult for any intruder to split up the public components ag and bg.
	\item Any attacker can combine ag+bg = abgg (extra bit) - too hard to figure out.
\end{itemize}
...
\subsection{Problems with Diffie-Hellman Key Exchange}
